...
namespace BT_LINQ
{
    public class Bai13
    {
        public static void Main(string[] args)
        {
            ...
            var firstStudent = students.FirstOrDefault(s => s.Score > 7);
            if (firstStudent != null)
            {
                Console.WriteLine("Sinh viên đầu tiên có điểm > 7:");
                Console.WriteLine($"{firstStudent.Name}: {firstStudent.Score}");
            }
            else
            {
                Console.WriteLine("Không có sinh viên nào có điểm > 7");
            }
        }
    }
}